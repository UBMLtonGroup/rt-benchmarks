...
object GCBench_multithread {
	...
	def comp_func(depth: Int, id: Int, iterations: Int, comp_sleep: Int): ListBuffer[String] = {
		...
		for(i <- 1 until iterations+1){
			...
			listOfTimeStamps += ("comp:start:"+ id +":" + i + ":" +tStart  + ":" + (runtime3.totalMemory - runtime3.freeMemory))
			fibonacci(depth)
			val tStop = System.currentTimeMillis()
                        val runtime4 = Runtime.getRuntime
			listOfTimeStamps += ("comp:stop:"+ id +":" + i + ":" +tStop  + ":" + (runtime4.totalMemory - runtime4.freeMemory))
